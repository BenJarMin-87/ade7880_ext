...
struct PowerChannel {
  ...
  // new
  void set_frequency(sensor::Sensor *sens) { this->frequency = sens; }
  void set_voltage_thd(sensor::Sensor *sens) { this->voltage_thd = sens; }
  void set_current_thd(sensor::Sensor *sens) { this->current_thd = sens; }
  ...
  sensor::Sensor *voltage_thd{nullptr};
  sensor::Sensor *current_thd{nullptr};
...
struct ADE7880ExtStore {
  volatile bool reset_done{false};
  bool reset_pending{false};
  ISRInternalGPIOPin irq1_pin;

  static void gpio_intr(ADE7880ExtStore *arg);
};

class ADE7880Ext : public i2c::I2CDevice, public PollingComponent {
 public:
 ...
 protected:
  ...
  void init_device_();
  void reset_device_();
  void select_harmonic_phase_(uint8_t phase);
