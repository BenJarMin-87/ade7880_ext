...
ade7880_ext_ns = cg.esphome_ns.namespace("ade7880_ext")
ADE7880Ext = ade7880_ext_ns.class_("ADE7880Ext", cg.PollingComponent, i2c.I2CDevice)
NeutralChannel = ade7880_ext_ns.struct("NeutralChannel")
PowerChannel = ade7880_ext_ns.struct("PowerChannel")
...
# extra keys
CONF_PHASE_FREQUENCY = "frequency"
...
POWER_CHANNEL_SCHEMA = cv.Schema(
    {
        ...
        cv.Optional(CONF_PHASE_FREQUENCY): cv.maybe_simple_value(
            sensor.sensor_schema(
        ...
        cv.Optional(CONF_VOLTAGE_THD): cv.maybe_simple_value(
            sensor.sensor_schema(
                unit_of_measurement=UNIT_PERCENT,
                accuracy_decimals=2,
                state_class=STATE_CLASS_MEASUREMENT,
        ...
        cv.Optional(CONF_CURRENT_THD): cv.maybe_simple_value(
            sensor.sensor_schema(
                unit_of_measurement=UNIT_PERCENT,
                accuracy_decimals=2,
                state_class=STATE_CLASS_MEASUREMENT,
...
def prefix_sensor_name(sensor_conf: ConfigType, channel_name: str, channel_config: ConfigType, sensor_type: str) -> None:
...
def process_channel_sensors(config: ConfigType, channel_key: str, sensor_types: tuple) -> None:
    if not (channel_config := config.get(channel_key)) or not (channel_name := channel_config.get(CONF_NAME)):
        return
...
CONFIG_SCHEMA = cv.All(
    ...
    cv.Schema(
        {
            cv.GenerateID(): cv.declare_id(ADE7880Ext),
